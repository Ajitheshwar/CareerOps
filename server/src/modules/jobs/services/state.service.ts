...
import { AgentOrchestrator } from '../agents/orchestrator.agent';
import { UserRepository } from '../../profile/repositories/profile.repository';
import { AgentState, AgentLog } from '../../../shared/types';
...
export class StateService {
  ...
  static initialize() {
    ...
    this.orchestrator.registerListener((state: any) => {
    ...
    this.orchestrator.registerLogListener((log: any) => {
  ...
  static async triggerSearch(
    ...
    this.orchestrator
      ...
      .catch((err: any) => console.error('Error in search pipeline:', err));
  }
  ...
  static async triggerTailor(jobId: string): Promise<void> {
    ...
    this.orchestrator
      .runTailoring(jobId)
      .catch((err: any) => console.error('Error in tailoring pipeline:', err));
  }
  ...
  static async triggerPrepInterview(jobId: string): Promise<void> {
    ...
    this.orchestrator
      ...
      .catch((err: any) => console.error('Error in interview prep pipeline:', err));
  }
}
