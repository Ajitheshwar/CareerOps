...
import { JobListing } from '../../../shared/db';
import { ResumeAnalyzerAgent } from '../agents/resume-analyzer.agent';
import { UserRepository } from '../../profile/repositories/profile.repository';
import { Job, MatchResult } from '../../../shared/types';
...
export class JobService {
  ...
  static async addAndAnalyzeJob(jobData: {
    ...
    const matchResult = await analyzer.run(
      ...
      job.company,
      job.description,
      (log: any) => console.log(`[JobService AddJob] [${log.level}] ${log.message}`)
    );
